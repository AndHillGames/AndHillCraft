package de.andhillgames.andhillcraft.container;
...
public class ContainerAdvancedCraftingTable extends Container{
	...
	public ContainerAdvancedCraftingTable(EntityPlayer player) {
	
		craftMatrix = new InventoryCrafting(this, 3, 3);
		craftResult = new InventoryCraftResult();
		this.chestInventory = new InventoryPlayer(player);
		...
		for (int i = 0; i < 3; i++)
        {
            for (int k = 0; k < 9; k++)
            {
                this.addSlotToContainer(new Slot(chestInventory, k + i * 9 + 9, 8 + k * 18, 84 + i * 18));
            }
        }
		...
		for (int i = 0; i < 3; i++)
        {
            for (int k = 0; k < 9; k++)
            {
                this.addSlotToContainer(new Slot(player.inventory, k + i * 9 + 9, 8 + k * 18, 151 + i * 18));
            }
        }
		
		for (int i = 0; i < 9; i++)
        {
            this.addSlotToContainer(new Slot(player.inventory, i, 8 + i * 18, 209));
        }
